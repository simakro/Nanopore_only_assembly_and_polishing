...
rule all:
    input:
        ...
        expand("results/{experiment}/{barcode}/medaka_{assembler}_pilon2_circos/{experiment}_{barcode}_circos.svg", experiment=EXPERIMENT, barcode=BARCODES, assembler=ASSEMBLERS),
        expand("results/{experiment}/{barcode}/assemblytics/{barcode}_medaka_{assembler}_pilon2.oriented.Assemblytics.Dotplot_filtered.png", experiment=EXPERIMENT, barcode=BARCODES, assembler=ASSEMBLERS)
...
rule dnadiff:
    ...
    output:
        ...
        "results/{experiment}/{barcode}/dnadiff/medaka_{assembler}_pilon2.oriented.delta"
    params:
...
checkpoint confirm_or_get_reference:
    ...
    run:
        ...
        os.makedirs(f"results/{EXPERIMENT}/medaka_{wildcards.assembler}_pilon2_gtdbtk_dwnld", exist_ok=True)
        for ref in dwnld:
...
rule plasclass:
    input:
        asm = "results/{experiment}/{barcode}/medaka_{assembler}_pilon2/medaka_{assembler}_pilon2.fasta" # consider using .oriented
    output:
        # "results/{experiment}/{barcode}/circl_fixstart/{assembly}.oriented.fasta"#,
        # directory("results/{experiment}/{barcode}/medaka_{assembler}_pilon2_gtdbtk")
...
rule mummer2circos_single:
    ...
    output:
        "results/{experiment}/{barcode}/medaka_{assembler}_pilon2_circos/{experiment}_{barcode}_circos.svg"
    params:
        # prefix="results/{experiment}/{barcode}/medaka_{assembler}_pilon2_circos/{experiment}_{barcode}_circos",
        prefix="{experiment}_{barcode}_circos",
        workdir="results/{experiment}/{barcode}/medaka_{assembler}_pilon2_circos"
    threads:
        4
    conda:
    ...
    log:
        "logs/{experiment}/{barcode}/medaka_{assembler}_pilon2/mummer2circos.log"
    shell:
        # "circos -outputfile {output} -conf circos_data/circos.config"
...
rule assemblytics:
    input:
        "results/{experiment}/{barcode}/dnadiff/medaka_{assembler}_pilon2.oriented.delta"
    output:
        "results/{experiment}/{barcode}/assemblytics/{barcode}_medaka_{assembler}_pilon2.oriented.Assemblytics.Dotplot_filtered.png"
    params:
        output_prefix="results/{experiment}/{barcode}/assemblytics/{barcode}_medaka_{assembler}_pilon2.oriented",
...
# add a third round of pilon polishing
...
# Quality assessment (QUAST or similar); probably covered by assemblytics
